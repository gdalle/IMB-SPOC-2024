...
md"""
# Deep learning meets combinatorial optimization

_A tale of missing gradients_
"""
...
SPOC seminar -- January 24th, 2024
	""",
	md"""
> Link: [gdalle.github.io/IMB-SPOC-2024/](https://gdalle.github.io/IMB-SPOC-2024)
	"""
)
...
function mysqrt(x::Real, k::Integer=10)
	y = x
	for _ in 1:k
		y = (y + x/y) / 2
	end
...
# ╔═╡ 5087b05e-2615-4544-88e0-79566a94c411
md"""
Its value and derivative at $x_0=2$
"""

# ╔═╡ 0a705ab6-b260-4a3e-bfb3-acaff9316afb
x0 = 2.0

# ╔═╡ a9acfb08-02d1-44e6-a60e-6acc7600a0ae
mysqrt(x0)

# ╔═╡ 27ca640b-a76d-49a3-9bff-84f4d33f7d6f
inv(2 * mysqrt(x0))

# ╔═╡ 5ee927bb-7f98-4ba0-9828-434cddb6d65e
md"""
## Manual differentiation
"""

# ╔═╡ e75fbdb8-db98-47a7-aca7-b71b39ab00f2
md"""
!!! info "Principle"
	Write down derivatives by hand
"""

# ╔═╡ 3a6ce5ef-a9c9-42d6-949a-b5a5649bae73
md"""
...
# ╔═╡ 51d60be3-6a93-4dce-8567-4d214ea0bd85
md"""
!!! danger "Main flaw"
	Labor intensive
"""

# ╔═╡ d6151465-2b1d-4609-ac8e-9e1e53d98e84
md"""
## Numerical differentiation
"""

# ╔═╡ 120e1c74-c5a0-405b-93c8-441a3e470d50
md"""
!!! info "Principle"
	$$f'(x_0) \approx \frac{f(x_0 + h) - f(x_0)}{h}$$
"""

# ╔═╡ 61be33f4-8233-494c-9bf2-7a0a10689034
md"""
 $\log_{10} h =$ $(@bind logh Slider(-15:0.1:0; default=0, show_value=true))
"""
...
# ╔═╡ 14b262c2-e7d5-4339-973d-827390df52e5
md"""
!!! danger "Main flaw"
	Inexact due to approximation or floating point errors
"""

# ╔═╡ d92b2c65-59f0-48dd-8859-a2d56dbdf1a2
md"""
## Symbolic differentiation
"""

# ╔═╡ d59acb62-f8ae-4485-aaa3-a181c2b97056
md"""
!!! info "Principle"
	Ask Wolfram Alpha (or an open source alternative)
"""

# ╔═╡ 8223482b-1462-472f-88ca-52ecf57436f9
md"""
 $k =$ $(@bind k Slider(0:10; show_value=true))
"""

# ╔═╡ 121ff14a-99f6-473c-b381-5796c48b3cf2
let
	@variables x
	mysqrt(x, k)
end

# ╔═╡ caeade05-f215-4e6f-9750-c537d7431cec
let
	@variables x
	Differential(x)(mysqrt(x, k)) |> expand_derivatives
end

# ╔═╡ ee9ab955-43f5-40fa-a144-398650ec5af5
md"""
!!! danger "Main flaw"
	Expression swell
"""

# ╔═╡ b6fa02c1-90ef-47b4-bb10-6128976335ed
md"""
## Forward mode algorithmic differentiation
"""

# ╔═╡ c8a3d190-8e7d-45c7-bc59-aa83af0d3fab
md"""
!!! info "Principle"
	Propagate both the number and its derivative forward through each function step
"""

# ╔═╡ 230b7023-fbb1-4e17-89ba-c91e118a0fd0
ForwardDiff.derivative(mysqrt, x0)

# ╔═╡ ba607f4f-1e04-4837-9dd5-cda4fb8e6c0b
function mysqrt_mean(x::AbstractVector, k::Integer=10)
	@info "Function call"
	return mean(mysqrt, x)
end

# ╔═╡ ab1f0218-3f9e-4d51-8320-ddc31f7ce20a
ForwardDiff.gradient(mysqrt_mean, rand(100))

# ╔═╡ 37ca8568-ec94-4fda-9a1c-1ebadaff1e8f
md"""
!!! danger "Main flaw"
	Slow to compute gradients (requires several function calls)
"""

# ╔═╡ 25711f25-4acb-4a8c-a39f-d321a303fe3f
md"""
## Reverse mode algorithmic differentiation
"""

# ╔═╡ 74ed2fe4-1f61-4c5d-a77f-8fadeb36fae8
md"""
!!! info "Principle"
...
md"""
!!! danger "Main flaw"
	Memory requirements scale linearly with the computation graph
"""

# ╔═╡ 752306ff-9f97-4297-a1c7-da743830f680
md"""
## Algorithmic differentiation is limited too
"""

# ╔═╡ 3cb84640-dcbf-4136-8959-24f1ad7c0e13
md"""
It interprets source code in a baroque way... but sometimes it cannot.
"""
...
function grid_graph(costs::AbstractMatrix{R}) where {R}
    h, w = size(costs)
    V = h * w
    E = (h - 2) * (w - 2) * 8 + (2(h - 2) + 2(w - 2)) * 5 + 4 * 3
    ...
    return SimpleWeightedGraph(sources, destinations, weights; combine=max)
end
...
function get_path(parents::AbstractVector{<:Integer}, s::Integer, d::Integer)
    ...
    while v != s
        v = parents[v]
        pushfirst!(path_sequence, v)
    end
    return path_sequence
end

# ╔═╡ e9bd63ab-abe0-4a76-8742-fcbd36c935aa
function shortest_path(costs::AbstractMatrix)
	h, w = size(costs)
	g = grid_graph(costs)
	result = dijkstra_shortest_paths(g, 1)
	path_sequence = get_path(result.parents, 1, h * w)
	path_matrix = zeros(Int, h, w)
	for v in path_sequence
...
function plot_grid(costs::AbstractMatrix, path::AbstractMatrix)
	h, w = size(costs)
	g = SimpleGraph(h * w)
	locs = index_to_coord.(vertices(g), h, w)
	locs_x = last.(locs)
	locs_y = first.(locs)
	edgelinewidth = 0
	nodefillc = weighted_color_mean.(vec(path), colorant"blue", colorant"white")
	gplot(g, locs_x, locs_y; edgelinewidth, nodefillc)
end

# ╔═╡ 2def6ea8-2790-4877-a221-14fbc5c2519c
function plot_path(path::AbstractMatrix)
	pl = heatmap(path[end:-1:1,:])
	plot!(pl, aspect_ratio=:equal, showaxis=false)
	return pl
end
